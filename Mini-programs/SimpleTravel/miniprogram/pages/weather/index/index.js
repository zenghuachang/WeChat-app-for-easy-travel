...
// 300-500 严重污染：有毒物
// 500以上 爆表：有毒物
let messages = require('../../../utils/weather/messages.js')
let bmap = require('../../../utils/weather/bmap-wx.js')
let utils = require('../../../utils/weather/utils')
...
Page({
  data: {
    ...
    message: '',
    cityDatas: {},
    icons: ['/images/weather/clothing.png', '/images/weather/carwashing.png', '/images/weather/pill.png', '/images/weather/running.png', '/images/weather/sun.png'],
    // 用来清空 input
    searchText: '',
    // 是否已经弹出
    hasPopped: false,
    ...
    bcgImgList: [
      {
        src: '/images/weather/xingkong_01.png',
        topColor: '#393836'
      },
      {
        src: '/images/weather/xingkong_02.png',
        topColor: '#0085e5'
      },
      {
        src: '/images/weather/xingkong_03.png',
        topColor: '#2d2225'
      },
      {
        src: '/images/weather/xingkong_04.png',
        topColor: '#004a89'
      }
  ...
  success (data) {
    ...
    let now = new Date()  //new 一个date对象然后调用
    // 存下来源数据
  ...
  commitSearch (res) {
    //console.log(res);
    let val = ((res.detail || {}).value || '').replace(/\s+/g, '') //js的字符串替换函数，这个函数有两个参数，第一个函数可以是字符串或正则表达式（用以进行匹配的子字符串），第二个函数是字符串（替换匹配的子字符串）....将原字符串中的所有空白字符替换成""，比如"abc d efg "字样的字符串使用这个函数后将变成"abcdefg"
    this.search(val)
  },
  dance() {
    this.setData({
  ...
  geocoder (address, success) {
    wx.request({
      ...
      success (res) {
        ...
        let data = res.data || {} //o = o || {} 表示：如果o为null或undefined，则将o初始化空对象（即{}），否则o不变。目的是防止o为null或未定义的错误。
        if (!data.status) {
          let location = (data.result || {}).location || {}
          // location = {lng, lat}
          success && success(location)//如果&&前面的表达式返回false，后面的部分就不会执行了。短路运算符
        } else {
          wx.showToast({
  ...
  menuOne () {
    this.menuMain()
    wx.navigateTo({
      url: '/pages/weather/switchcity/switchcity',
    })
  },
  menuTwo () {
    this.menuMain()
    wx.navigateTo({
      url: '/pages/weather/citychoose/citychoose',
    })
  },
  menuThree () {
    this.menuMain()
